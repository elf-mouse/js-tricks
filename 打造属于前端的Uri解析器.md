...
基本思路：使用 match 方法，从目标字符串中匹配与 key 对应的参数的值并返回。使用正则表达式匹配，可以省去循环，可以说是第二种 split 用法的升级版（仅从省代码考虑，性能恐怕未必），具体代码如下：
...
OK，结果正常了！由此，也可以映射一个问题，当我们在地址栏传递数据时，还是尽可能的 encode 好后再使用，这样可以避免一些不必要的麻烦。
